  ...
  return (
    <section className="account-dashboard" aria-labelledby="account-heading">
      ...
      <article className="account-profile">
        ...
        <dl className="account-details"><div><dt>Stav účtu</dt><dd>Aktívny</dd></div><div><dt>E-mail</dt><dd>{emailStatus}</dd></div><div><dt>Typ členstva</dt><dd>{storedMembership}</dd></div><div><dt>Stav členstva</dt><dd>{membershipStatusLabels[membershipStatus]}</dd></div><div><dt>Aktívny prístup</dt><dd>{membershipLabels[effectiveMembership]}</dd></div><div><dt>Platnosť</dt><dd>{formatMembershipDate(profile.membership_expires_at)}</dd></div>{hasPaidAccess && <div><dt>Automatické obnovenie</dt><dd className={cancellationScheduled ? 'is-cancelled' : ''}>{cancellationScheduled ? 'Zrušené' : 'Aktívne'}</dd></div>}<div><dt>Začiatok členstva</dt><dd>{formatMembershipDate(profile.membership_started_at)}</dd></div><div><dt>Registrácia</dt><dd>{formatMembershipDate(profile.created_at)}</dd></div></dl>
      </article>
