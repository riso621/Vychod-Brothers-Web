...
function PlayerIcon({ name }) {
  const paths = {
    ...
    muted: <><path d="M5 10h3l4-3v10l-4-3H5v-4Z" /><path d="m16 9 4 6M20 9l-4 6" /></>,
    volume: <><path d="M5 10h3l4-3v10l-4-3H5v-4Z" /><path d="M16 9c1.7 1.7 1.7 4.3 0 6M18.5 6.5c3.2 3.2 3.2 7.8 0 11" /></>,
    settings: <><circle cx="12" cy="12" r="3" /><path d="M19 13.5v-3l-2-.7-.7-1.7.9-1.9-2.1-2.1-1.9.9-1.7-.7L10.8 2h-3l-.7 2.3-1.7.7-1.9-.9-2.1 2.1.9 1.9-.7 1.7-2 .7v3l2 .7.7 1.7-.9 1.9 2.1 2.1 1.9-.9 1.7.7.7 2.3h3l.7-2.3 1.7-.7 1.9.9 2.1-2.1-.9-1.9.7-1.7 2-.7Z" transform="scale(.72) translate(4.7 4.7)" /></>,
    fullscreen: <path d="M4 9V4h5M15 4h5v5M20 15v5h-5M9 20H4v-5" />,
  }
  return <svg className="video-control-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>
}

function CloudflareTrackedPlayer({ playerUrl, title, videoId, watchProgress, onWatchProgress, autoPlay = false, startMuted = false, playbackActive = true, replaySignal = 0, poster = '', onDurationChange, onTimeProgress, onEnded }) {
  ...
  useEffect(() => {
    ...
    if (startMuted && !initialMuteAppliedRef.current) {
      video.muted = true
    ...
    const handleMetadata = () => {
      updateSnapshot()
      onDurationChange?.(Number(video.duration) || 0)
      if (resumePosition) {
    ...
    const handleTimeUpdate = () => { updateSnapshot(); onTimeProgress?.({ currentTime: Number(video.currentTime) || 0, duration: Number(video.duration) || 0 }); persist() }
    ...
    const handleEnded = () => { setPaused(true); revealControls(); updateSnapshot(); persist(true, true); onEnded?.() }
    const handleCanPlay = () => {
      if (autoPlay && playbackActiveRef.current && video.paused) video.play().catch(() => {})
    }
  ...
  }, [autoPlay, manifestUrl, onDurationChange, onEnded, onTimeProgress, onWatchProgress, poster, resumePosition, revealControls, startMuted, videoId])

  useEffect(() => {
    const video = videoRef.current
    if (!video || !autoPlay) return
    if (!playbackActive) video.pause()
    else if (video.readyState >= 2 && video.paused && !video.ended) video.play().catch(() => {})
  ...
  return <div className={`video-detail-stage video-player-cloudflare ${controlsVisible ? 'controls-visible' : 'controls-hidden'}`} data-stream-video-id={videoId || undefined} data-audio-tracks={audioTrackCount} onPointerMove={() => revealControls()} onPointerDown={() => revealControls()} onKeyDown={() => revealControls()}><video ref={videoRef} title={`Prehrať video ${title}`} playsInline preload="metadata" onClick={handleTogglePlayback}>Tvoj prehliadač nepodporuje prehrávanie videa.</video><div className={`video-custom-controls${controlsVisible ? ' is-visible' : ''}`}><input className="video-seek" type="range" min="0" max={duration || 0} step="0.1" value={Math.min(currentTime, duration || 0)} onChange={handleSeek} onPointerMove={() => revealControls()} aria-label="Pozícia videa" /><div className="video-control-row"><button type="button" onClick={handleTogglePlayback} aria-label={paused ? 'Prehrať video' : 'Pozastaviť video'}><PlayerIcon name={paused ? 'play' : 'pause'} /></button><span className="video-time">{formatTime(currentTime)} / {formatTime(duration)}</span><div className="video-volume"><button type="button" onClick={handleMute} aria-label={muted || volume === 0 ? 'Zapnúť zvuk' : 'Stlmiť zvuk'}><PlayerIcon name={muted || volume === 0 ? 'muted' : 'volume'} /></button><input type="range" min="0" max="1" step="0.05" value={muted ? 0 : volume} onChange={handleVolume} aria-label="Hlasitosť" /></div><div className="video-controls-spacer" />{qualityLevels.length > 0 && <div className="video-settings"><button type="button" className={settingsOpen ? 'is-active' : ''} onClick={handleSettingsToggle} aria-label={`Nastavenia kvality, ${selectedQualityLabel}`} aria-expanded={settingsOpen}><PlayerIcon name="settings" /></button>{settingsOpen && <div className="video-quality-menu" role="menu" aria-label="Kvalita videa"><strong>Kvalita</strong><button type="button" role="menuitemradio" aria-checked={selectedQuality === 'auto'} onClick={() => handleQualityChange('auto')}><span>{selectedQuality === 'auto' ? '✓' : ''}</span>Auto</button>{qualityLevels.map((level) => { const value = String(level.index); return <button type="button" role="menuitemradio" aria-checked={selectedQuality === value} onClick={() => handleQualityChange(value)} key={level.height}><span>{selectedQuality === value ? '✓' : ''}</span>{level.height}p</button> })}</div>}</div>}<button type="button" onClick={handleFullscreen} aria-label="Prepnúť video na celú obrazovku"><PlayerIcon name="fullscreen" /></button></div></div>{resumePosition > 0 && <span className={`video-resume-notice${resumeToastVisible ? ' is-visible' : ''}`}>Pokračujeme od {formatTime(resumePosition)}</span>}</div>
}

export default function VideoPlayer({ youtubeUrl, title, accessLevel, streamVideoId, provider = 'none', poster = '', previewImage = '', hasAccess = accessLevel === 'free', accessLoading = false, lockedCopy = null, videoId = '', watchProgress = null, onWatchProgress = null, trailer = false, autoPlay = false, startMuted = false, playbackActive = true, replaySignal = 0, onDurationChange, onTimeProgress, onEnded }) {
  const youtubeVideoId = provider === 'youtube' && hasAccess ? getYoutubeVideoId(youtubeUrl) : null
  const accessMessage = 'Celé video je dostupné aktívnym členom Východ Brothers Club'
  ...
  const { url: cloudflarePlayerUrl, loading: cloudflareLoading } = useCloudflarePlaybackUrl(streamVideoId, provider === 'cloudflare_stream' && hasAccess && !accessLoading, trailer)

  if (accessLoading) return <PlayerState heading="Overujeme prístup…" accessLevel={accessLevel} image={image} />
  if (!hasAccess) return <PlayerState heading={lockedCopy?.heading || 'Členský obsah'} description={lockedCopy?.description || 'Odomkni si celý svet Východ Brothers a sleduj obsah bez obmedzení.'} accessMessage={accessMessage} accessLevel={accessLevel} image={image} locked ctaLabel={lockedCopy?.ctaLabel} ctaHref={lockedCopy?.ctaHref} />
  ...
  if (provider === 'cloudflare_stream') {
    ...
    return <CloudflareTrackedPlayer key={cloudflarePlayerUrl} playerUrl={cloudflarePlayerUrl} title={title} videoId={videoId} watchProgress={watchProgress} onWatchProgress={onWatchProgress} autoPlay={autoPlay} startMuted={startMuted} playbackActive={playbackActive} replaySignal={replaySignal} poster={image || ''} onDurationChange={onDurationChange} onTimeProgress={onTimeProgress} onEnded={onEnded} />
  }
