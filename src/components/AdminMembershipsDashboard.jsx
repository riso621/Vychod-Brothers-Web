...
export default function AdminMembershipsDashboard() {
  ...
  return <section className="admin-videos admin-memberships"><header className="admin-videos-heading"><div><span>ADMIN / ČLENSTVÁ</span><h1>Členstvá</h1><p>Správa úrovní, expirácie a stavu používateľských účtov.</p></div></header>{error && <p className="admin-videos-status is-error" role="alert">{error}</p>}<div className="admin-members-list">{users.map((user) => <MembershipUser user={user} key={user.id} onChanged={(updated) => setUsers((current) => current.map((item) => item.id === updated.id ? { ...item, ...updated } : item))} />)}</div></section>
}
