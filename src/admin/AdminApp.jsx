...
function Dashboard({ users, videos, invoices, loading }) {
  ...
  const paidMonth = invoices.filter((i) => i.paid && i.created >= monthStart)
...
function Payments({ users, invoices, loading }) {
  const customerMap=new Map(users.map((u)=>[u.stripe_customer_id,u])); const paid=invoices.filter((i)=>i.paid),failed=invoices.filter((i)=>['open','uncollectible','void'].includes(i.status)); const monthStart=new Date(new Date().getFullYear(),new Date().getMonth(),1).getTime()/1000; const revenue=invoices.filter((i)=>i.paid&&i.created>=monthStart).reduce((s,i)=>s+i.amount_paid,0)
  return <section><PageHeading eyebrow="ADMIN / BILLING" title="Platby" text="Reálne faktúry načítané zo Stripe bez citlivých platobných údajov."/><div className="admin-metrics"><Metric label="Tržby tento mesiac" value={money(revenue,'eur')} detail="zaplatené faktúry"/><Metric label="Úspešné platby" value={paid.length} detail="v načítanom období"/><Metric label="Neúspešné platby" value={failed.length} detail="open / failed"/><Metric label="Naplánované zrušenia" value={users.filter((u)=>u.stripe_cancel_at_period_end).length} detail="prístup ostáva aktívny"/></div>{loading?<AdminLoading/>:invoices.length?<div className="admin-table-wrap"><table><thead><tr><th>Používateľ</th><th>Plán</th><th>Suma</th><th>Dátum</th><th>Stav</th><th>Typ</th><th>Invoice ID</th></tr></thead><tbody>{invoices.map((i)=>{const u=customerMap.get(i.customer);return <tr key={i.id}><td><strong>{u?.email||'Nepriradený zákazník'}</strong></td><td>{u?.membership?.toUpperCase()||'—'}</td><td>{money(i.amount_paid||i.amount_due,i.currency)}</td><td>{formatDate(i.created*1000)}</td><td>{i.status}</td><td>{i.type}</td><td><small>{i.id}</small></td></tr>})}</tbody></table></div>:<Empty text="Stripe nevrátil žiadne faktúry."/>}</section>
}

function UserDetail({ userId }) { const [data,setData]=useState(null),[error,setError]=useState(''); useEffect(()=>{adminRequest({action:'user-detail',userId}).then(setData).catch((e)=>setError(e.message))},[userId]); if(error)return <Empty text={error}/>; if(!data)return <AdminLoading/>; const u=data.user; return <section><button className="admin-back" onClick={()=>navigate('/admin/users')}>← Používatelia</button><PageHeading eyebrow="ADMIN / USER DETAIL" title={u.username||u.email||'Používateľ'} text={u.email||u.id}/><div className="admin-detail-grid"><article className="admin-panel"><h2>Základné údaje</h2><dl className="admin-detail-list"><div><dt>User ID</dt><dd>{u.id}</dd></div><div><dt>E-mail</dt><dd>{u.email||'—'}</dd></div><div><dt>Registrácia</dt><dd>{formatDate(u.created_at)}</dd></div><div><dt>Posledné prihlásenie</dt><dd>{formatDate(u.last_sign_in_at)}</dd></div><div><dt>E-mail</dt><dd>{u.email_confirmed_at?'Overený':'Neoverený'}</dd></div></dl></article><article className="admin-panel"><h2>Členstvo</h2><dl className="admin-detail-list"><div><dt>Plán</dt><dd>{u.membership.toUpperCase()}</dd></div><div><dt>Stav</dt><dd>{u.membership_status}</dd></div><div><dt>Stripe</dt><dd>{u.stripe_subscription_status||'Bez subscription'}</dd></div><div><dt>Obnova</dt><dd>{u.stripe_cancel_at_period_end?'Zrušená':'Aktívna'}</dd></div><div><dt>Platnosť</dt><dd>{formatDate(u.membership_expires_at)}</dd></div></dl></article></div><article className="admin-panel admin-detail-section"><h2>Platby</h2>{data.invoices.length?<div className="admin-compact-list">{data.invoices.map((i)=><div key={i.id}><strong>{money(i.amount_paid||i.amount_due,i.currency)} · {i.status}</strong><span>{formatDate(i.created*1000)} · {i.type}</span></div>)}</div>:<Empty text="Žiadne platby"/>}</article><article className="admin-panel admin-detail-section"><h2>História pozerania</h2><p>{data.watchHistory.length} videí v histórii</p></article></section> }
